...
logistic_regression['hyperparameters'] = 'max_iter: '+logistic_regression['max_iter'].astype(str) + 'solver: '+logistic_regression['solver'].astype(str)
logistic_regression_dropped = logistic_regression.drop(['max_iter', 'solver'], axis=1)
...
xgboost['hyperparameters'] = 'n_jobs: '+xgboost['n_jobs'].astype(str) + 'max_depth: '+xgboost['max_depth'].astype(str) + 'learning_rate: '+xgboost['learning_rate'].astype(str)
xgboost_dropped = xgboost.drop(['n_jobs', 'max_depth', 'learning_rate'], axis=1)
...
combined_cross_validation_data = pd.concat([logistic_regression_dropped, decision_tree_dropped, random_forest_dropped, adaptive_boosting_dropped, svm_dropped])
...
combined_cross_validation_data.to_csv('charts/combined_results.csv', index=None)
